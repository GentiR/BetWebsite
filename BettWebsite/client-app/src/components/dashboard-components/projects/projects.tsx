...
interface Laliga{
  ...
  const Projects = () => {
    const [data, setData] = React.useState<Laliga[]>([] as Laliga[]);
    const [selectedData, setSelectedData] = React.useState<Laliga>({} as Laliga);
...
React.useEffect(() => {
    axios
    .get("http://localhost:5000/api/Laliga")
    ...
    const openInsert = () => {
      ...
      const openDeleteModal = (item: Laliga) => {
        setSelectedData(item);
        setFormState("delete");
        setOpenModal(true);
      };
    
      const openEditModal = (item: Laliga) => {
        setSelectedData(item);
        setFormState("edit");
        setOpenModal(true);
      };
      ...
      const addBett = (item: Laliga) => {
        axios
          .post("http://localhost:5000/api/Laliga/insertBett", item)
          .then((res) => {
      ...
      const deleteBett = () => {
        axios
          .delete(
            "http://localhost:5000/api/Laliga/deleteBett/" + selectedData.matchId
          )
          .then((res) => {
            if (res.data !== 400)
      ...
      const editBett = (bett: Laliga) => {
        axios
          .put("http://localhost:5000/api/Laliga/editBett/" + 
          bett.matchId, 
          bett
          )
