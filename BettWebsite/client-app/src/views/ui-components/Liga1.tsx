...
interface Liga1{
  ...
  const Projects = () => {
    const [data, setData] = React.useState<Liga1[]>([] as Liga1[]);
    const [selectedData, setSelectedData] = React.useState<Liga1>({} as Liga1);
    ...
    const openInsert = () => {
      ...
      const openDeleteModal = (item: Liga1) => {
        setSelectedData(item);
        setFormState("delete");
        setOpenModal(true);
      };
    
      const openEditModal = (item: Liga1) => {
        setSelectedData(item);
        setFormState("edit");
        setOpenModal(true);
      };
      ...
      React.useEffect(() => {
        axios
        .get("http://localhost:5000/api/Liga1")
        .then((res) => setData(res.data));
    });

      const addBett = (item: Liga1) => {
        axios
          .post("http://localhost:5000/api/Liga1/insertBett", item)
          .then((res) => {
      ...
      const deleteBett = () => {
        axios
          .delete(
            "http://localhost:5000/api/Liga1/deleteBett/" + selectedData.matchId
          )
          .then((res) => {
            if (res.data !== 400)
      ...
      const editBett = (bett: Liga1) => {
        axios
          .put("http://localhost:5000/api/Liga1/editBett/" + 
          bett.matchId, 
          bett
          )
