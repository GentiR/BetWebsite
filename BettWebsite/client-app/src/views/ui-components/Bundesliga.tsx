...
interface Bundesliga{
  ...
  const Projects = () => {
    const [data, setData] = React.useState<Bundesliga[]>([] as Bundesliga[]);
    const [selectedData, setSelectedData] = React.useState<Bundesliga>({} as Bundesliga);
    ...
    const openInsert = () => {
      ...
      const openDeleteModal = (item: Bundesliga) => {
        setSelectedData(item);
        setFormState("delete");
        setOpenModal(true);
      };
    
      const openEditModal = (item: Bundesliga) => {
        setSelectedData(item);
        setFormState("edit");
        setOpenModal(true);
      };
      ...
      React.useEffect(() => {
        axios
        .get("http://localhost:5000/api/Bundesliga")
        .then((res) => setData(res.data));
    });

      const addBett = (item: Bundesliga) => {
        axios
          .post("http://localhost:5000/api/Bundesliga/insertBett", item)
          .then((res) => {
      ...
      const deleteBett = () => {
        axios
          .delete(
            "http://localhost:5000/api/Bundesliga/deleteBett/" + selectedData.matchId
          )
          .then((res) => {
            if (res.data !== 400)
      ...
      const editBett = (bett: Bundesliga) => {
        axios
          .put("http://localhost:5000/api/Bundesliga/editBett/" + 
          bett.matchId, 
          bett
          )
