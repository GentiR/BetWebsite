...
interface PremiereLeague{
  ...
  const Projects = () => {
    const [data, setData] = React.useState<PremiereLeague[]>([] as PremiereLeague[]);
    const [selectedData, setSelectedData] = React.useState<PremiereLeague>({} as PremiereLeague);
    ...
    const CloseModal = () => {
        setFormState("");
        setSelectedData({} as PremiereLeague);
        setOpenModal(false);
    };
    ...
    const openInsert = () => {
      ...
      const openDeleteModal = (item: PremiereLeague) => {
        setSelectedData(item);
        setFormState("delete");
        setOpenModal(true);
      };
    
      const openEditModal = (item: PremiereLeague) => {
        setSelectedData(item);
        setFormState("edit");
        setOpenModal(true);
      };
      ...
      React.useEffect(() => {
        axios
        .get("http://localhost:5000/api/PremierLeague")
        .then((res) => setData(res.data));
    });

      const addBett = (item: PremiereLeague) => {
        axios
          .post("http://localhost:5000/api/PremierLeague/insertBett", item)
          .then((res) => {
      ...
      const deleteBett = () => {
        axios
          .delete(
            "http://localhost:5000/api/PremierLeague/deleteBett/" + selectedData.matchId
          )
          .then((res) => {
            if (res.data !== 400)
      ...
      const editBett = (bett: PremiereLeague) => {
        axios
          .put("http://localhost:5000/api/PremierLeague/editBett/" + 
          bett.matchId, 
          bett
          )
